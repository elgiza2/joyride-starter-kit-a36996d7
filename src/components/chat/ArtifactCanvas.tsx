...
export default function ArtifactCanvas({
  ...
  const previewHtml = useMemo(
    () => buildProjectPreviewHtml(files) || (isReactProject(files) ? buildReactRuntimeHtml(files, "Preview") : null),
    [files],
  );
                    ...
                    <Button
                      ...
                      onClick={() => {
                        window.dispatchEvent(new CustomEvent("megsy:prefill-composer", {
                          detail: { text: `أصلح هذا الخطأ في المشروع:\n\n${runtimeError}` },
                        }));
                        onOpenChange(false);
                        toast.success("تم نقل الخطأ إلى الكتابة");
                      }}
                    >
                      Fix with AI
                    </Button>
