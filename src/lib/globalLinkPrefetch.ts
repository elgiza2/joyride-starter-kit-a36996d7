...
export function installGlobalLinkPrefetch() {
  ...
  // pointerdown fires ~80ms before click — grabs the chunk the instant the
  // user commits to tapping, even on devices with no hover state.
  window.addEventListener("pointerdown", onIntent, { passive: true, capture: true });
